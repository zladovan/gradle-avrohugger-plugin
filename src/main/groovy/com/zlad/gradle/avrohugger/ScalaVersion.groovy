...
/**
 * Callable returning used scala version in partial form MAJOR.MINOR (e.g. 2.13).
 *
...
class ScalaVersion implements Callable<String> {
    ...
    private static final SCALA_VERSION_2_13 = '2.13'
    ...
    private static String findScalaVersion(Project project) {
        ...
        // if version is still not found give up and pretend that version is 2.13 (no 2.10 backward compatibility)
        if (version == null) {
            logger.warn(
                "Scala library dependency was not found. Using `$SCALA_VERSION_2_13` as default fallback."
            )
            return SCALA_VERSION_2_13
        }
    	...
    	logger.warn(
            "Invalid scala library version was found: `$version`. Using `$SCALA_VERSION_2_13` as default fallback."
        )
        return SCALA_VERSION_2_13
    }
